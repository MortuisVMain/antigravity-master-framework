...
* **Механика работы:**
  ...
  4. Если предметная область — обучение ребенка, поднимает **Педагогический протокол (Правило 22: Модель «мешок и конфеты»)**.
...
| **5** | **🎓 Обучение & Педагогика** | *Правило 22 (Школьный стандарт 10 лет, 5 класс)* | Модель «мешок с конфетами», задачи в 2-3 действия, скрытые ответы (anti-cheat UI). |
...
## 🛡️ 5. Золотое Правило Интеграции
Критики **НИКОГДА** не пишут код напрямую в рабочие файлы. Они обладают правами аудиторов (read-only). Правом внесения изменений обладает только основной агент после прохождения гейта верификации и одобрения пользователя.
